...
class TestPoolDeleted:
    ...
    @pytest.mark.asyncio
    async def test_delete_other_error_is_logged(self, mock_manager, caplog):
        mock_manager.custom_api.delete_namespaced_custom_object.side_effect = ApiException(status=500)
        with patch("controller.pool.get_manager", return_value=mock_manager):
            with patch("controller.pool.get_pool_sandboxes", AsyncMock(return_value=[
                {"metadata": {"name": "s1"}},
            ])):
                with caplog.at_level(logging.ERROR, logger="controller.pool"):
                    await pool.pool_deleted(name="pool", namespace="default", status={})
        assert "Failed to delete sandbox s1" in caplog.text


class TestSandboxEvent:
    @pytest.mark.asyncio
    async def test_no_pool_label_returns_early(self, caplog):
        with caplog.at_level(logging.DEBUG, logger="controller.pool"):
            await pool.sandbox_event(event={"type": "MODIFIED"}, name="s1", namespace="default", labels={})
        assert caplog.text == ""

    @pytest.mark.asyncio
    async def test_irrelevant_event_type_ignored(self, caplog):
        with caplog.at_level(logging.DEBUG, logger="controller.pool"):
    ...
    @pytest.mark.asyncio
    async def test_modified_without_claim_ignored(self, caplog):
        with caplog.at_level(logging.DEBUG, logger="controller.pool"):
            await pool.sandbox_event(
    ...
    @pytest.mark.asyncio
    async def test_modified_with_claim_logged(self, caplog):
        with caplog.at_level(logging.DEBUG, logger="controller.pool"):
            await pool.sandbox_event(
                event={
                    "type": "MODIFIED",
        ...
        assert "may require pool pool replenishment" in caplog.text

    @pytest.mark.asyncio
    async def test_deleted_event_logged(self, caplog):
        with caplog.at_level(logging.DEBUG, logger="controller.pool"):
            await pool.sandbox_event(
