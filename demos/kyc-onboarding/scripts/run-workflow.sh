...
show_usage() {
    ...
    echo ""
    echo "  Phase I - Ownership Structure:"
    echo "    ownership, os     → lx-retrieve-ownership-structure"
    echo "    controllers, kc   → lx-retrieve-key-controllers"
    echo ""
    echo "  Phase II - Entities & Assessment:"
    echo "    vessels, ev       → lx-retrieve-entities-vessels"
    echo "    purpose, por      → lx-assess-purpose-of-relationship"
    echo ""
    echo "  Phase III - Screening:"
    echo "    adverse, ams      → lx-adverse-media-screening"
    echo "    sanctions, bss    → lx-blacklist-sanction-screening"
    echo ""
    echo "Execution modes (optional, default: parallel):"
    echo "    fake              → Use fake data (fast testing)"
    echo "    seq               → Sequential extraction"
    echo "    parallel          → Parallel extraction (default)"
    echo ""
    echo "Examples:"
    echo "    $0 vessels fake"
    echo "    $0 ownership seq"
    echo "    $0 controllers"
    echo ""
    exit 1
}
