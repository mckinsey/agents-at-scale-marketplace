...
> **⚠️ Deprecated.** This standalone MCP server is deprecated and will be combined into the
> [File Gateway](/services/file-gateway) service. File Gateway bundles this MCP alongside
> the REST file-api and VersityGW and supports both the `filesystem` (PVC) and `s3`
> storage backends. Use File Gateway for new deployments; this standalone chart will be
